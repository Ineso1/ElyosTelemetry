...
import SatelliteMap from './SatelliteTrack';
...
function MapSection() {
  ...
  const onTrackPointClick = (trackPoint) => {
    setSelectedTrackPoint(trackPoint);
  };
  ...
  return (
    <div className="App">
      <button onClick={toggleView}>
        {showTrack ? 'Switch to Satellite View' : 'Switch to Track View'}
      </button>

      {showTrack ? (
        <Track data={trackData} onTrackPointClick={onTrackPointClick} />
      ) : (
        <SatelliteMap trackData={trackData} selectedTrackPoint={selectedTrackPoint} />
      )}
    </div>
  );
}
